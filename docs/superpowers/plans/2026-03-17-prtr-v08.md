...
func GitDiff(ctx context.Context, root string) (string, error) {
    raw, err := gitOutputDir(ctx, root, "diff", "HEAD")
    if err != nil {
        if isNotGitRepo(err) {
            return "", nil
        }
...
```go
// In Execute() or RunDeep() (whatever the entry function is), before graphFactory().Run():
...
// Replace opts.Progress with channel sender:
